...
def upload(start_date, end_date):
    ...
    feat_dict = {'ResDaBa.dbo.f_sray_features': apps['FeatureTotal'][(2, 6, 0, 0)],
                 'ResDaBa.dbo.f_sray_features_long': apps['FeatureLong'][(2, 6, 0, 0)],
                 'ResDaBa.dbo.f_sray_features_short': apps['FeatureShort'][(2, 6, 0, 0)]}

    for table, app in feat_dict.items():
        q = f"""
SELECT assetid,
 date,
 ...
 CAST(ROUND(bi_cou, 4) AS NUMERIC(10,4)) AS bi_cou
FROM {table}
 WHERE job_id = 77991
